<?php while (have_rows('testo')) : the_row(); ?>
    ...
    <div class="content <?php echo $padding ?>">
        ...
        <?php if (!get_sub_field('titolo_precompilato') && trim(get_sub_field('titolo')) != '') : ?>
            ...
            <h3 class="title <?php echo $align ?> <?php echo(get_sub_field('enfasi_titolo') ? ' emphasis' : '') ?> <?php the_sub_field('grandezza_titolo') ?>"><?php the_sub_field('titolo') ?>  <?php echo (the_sub_field('contenuto')!='') ? 'no-padding' : '' ?></h3>
        <?php else : ?>
